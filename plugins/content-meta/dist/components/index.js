  ...
  function ContentMetadata({ cfg, fileData, displayClass }) {
    ...
    if (text) {
      ...
      if (fileData.dates) {
        ...
        const createdDate = getDate({ ...fileData, defaultDateType: "created" });
        if (createdDate) {
          segments.push(/* @__PURE__ */ u2("span", { children: ["Created: ", /* @__PURE__ */ u2(DateComponent, { date: createdDate, locale })] }));
        }
        if (fileData.filePath) {
          try {
            const __fs = __require("fs");
            const st = __fs.statSync(fileData.filePath);
            segments.push(/* @__PURE__ */ u2("span", { children: ["Updated: ", /* @__PURE__ */ u2(DateComponent, { date: new Date(st.mtimeMs), locale })] }));
          } catch {}
        }
      }
